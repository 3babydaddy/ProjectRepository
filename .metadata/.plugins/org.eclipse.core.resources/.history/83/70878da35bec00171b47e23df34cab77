...
function editRow(){
	...
	var url = ctx + '/unpublic/partyorgedit?id='+id;
	//openWin("编辑", url,"90%","90%",function(){reloadData()});
...
function cancelRow(){
	...
	var url = ctx + '/unpublic/partyorgcancel?id='+id+'&method=edit';
...
function cancelReasonRow(id){
	...
	var url = ctx + '/unpublic/partyorgcancel?id='+id+'&method=look';
...
function delRow(id){
	...
	layer.confirm('您确认想要删除记录吗？',function(index){  
		layer.close(index);
    	$.ajax({
    		url:ctx + '/unpublic/partyorgdelete',
    		data:{id:id},
    		type:'post',
    		dataType:'json',
...
function nocancel(){
	...
	layer.confirm('您想要取消撤销审批吗？',function(){    
    	$.ajax({
    		url:ctx + '/unpublic/partyNocancel',
    		data:{id:id},
    		type:'post',
    		dataType:'json',
    		success:function(result){
...
function  cancelok(){
	...
	layer.confirm('确认撤销审批通过吗？',function(){    
		$.ajax({
    		url:ctx + '/unpublic/partyCancelok',
...
function returnRow(){
	...
	layer.confirm('您确认要退回操作吗？',function(){    
		$.ajax({
			url:ctx + '/unpublic/partyOrgSetStatus',
			data:{id:id , status:1},
			type:'post',
			dataType:'json',
			success:function(result){
...
function reportHigherRow(){
	...
	$.ajax({
		url:ctx + '/unpublic/partyOrgsSetStatus',
...
function reportHigherOption(orgId, sign){
	var url = ctx + '/unpublic/partyorgedit?id='+orgId+'&clickSign='+sign;
	utils.e.openWin('editwin','编辑',url,"80%","80%",function(){
		reloadData();
	});
}
