...
function loadData(){
	...
	var options = {
			...
			columns : [ [
			   ...
			   ,{field :"filename",title :"文件名称",width :"20%", align:"center"}
...
function uploadFile(modelTbId){
	var model = $("#modelFile").val();
	var type = $("#typeFile").val();
	var action = $("#actionFile").val();
	posterProcess1({
		scriptData : {modelTbId :modelTbId ,model:model,type:type,action:action},
		script : 'fileUploads',
		fileExt:'*.*',
...
function downRow(fileId){
	$("#hiddeFileIframe").attr("src",ctx + '/unpublic/downFiles?id='+fileId);
...
function closeJbox(){
	...
	var sign = $("#sign").val();
	window.parent.document.getElementById(sign).value=fileName;
	window.parent.document.getElementById(sign.replace('file','')).value=fileId;
	parent.utils.e.closeWin('cancelwin');
}