...
function loadData(){
	var options = {
			url : ctx+'/cover/partyconstructionlist',
...
function addRow(){
	...
	var partyOrgId = row.coverPartyOrgId;
	var url = ctx + '/cover/partyconstructionedit?id='+id+'&partyOrgId='+partyOrgId+'&partyOrgName='+partyOrgName;
...
function lookRow(){
	...
	var id = row.id;
	var partyOrgName = row.partyOrgName;
	var url = ctx + '/cover/partyconstructionlook?id='+id+'&partyOrgName='+partyOrgName;
...
function cancelRow(){
	...
	var url = ctx + '/cover/partyconstructioncancel?id='+id+'&method=edit';
...
function cancelReasonRow(id){
	...
	var url = ctx + '/cover/partyconstructioncancel?id='+id+'&method=look';
...
function delRow(id){
	...
	layer.confirm('您确认想要删除记录吗？',function(index){  
		layer.close(index);
    	$.ajax({
    		url:ctx + '/cover/partyconstructiondelete',
    		data:{id:id},
    		type:'post',
    		dataType:'json',
...
function nocancel(){
	...
	layer.confirm('您想要取消撤销审批吗？',function(){    
    	$.ajax({
    		url:ctx + '/cover/partyConstructionNocancel',
    		data:{id:id},
    		type:'post',
    		dataType:'json',
    		success:function(result){
...
function  cancelok(){
	...
	layer.confirm('确认撤销审批通过吗？',function(){    
		$.ajax({
    		url:ctx + '/cover/partyConstructionCancelok',
...
function returnRow(){
	...
	layer.confirm('您确认要退回操作吗？',function(){    
		$.ajax({
			url:ctx + '/cover/partyConstructionSetStatus',
			data:{id:id , status:1},
			type:'post',
			dataType:'json',
			success:function(result){
...
function reportHigherRow(){
	...
	$.ajax({
		url:ctx + '/cover/partyConstructionsSetStatus',
