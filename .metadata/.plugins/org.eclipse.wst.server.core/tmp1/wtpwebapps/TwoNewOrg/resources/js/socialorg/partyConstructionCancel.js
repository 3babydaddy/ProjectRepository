...
function loadData(){
	var options = {
			url : ctx+'/socialorg/partyShowlFiles',
...
function save(){
	$.ajax({
		url:ctx + '/socialorg/partyconstructioncancel',
		type:'post',
...
function uploadFile(modelTbId){
	posterProcess1({
		scriptData : {modelTbId :modelTbId ,model:"1",type:'2',action:'1'},
		script : 'fileUploads',
		fileExt:'*.*',
...
function downRow(fileId){
	$("#hiddeFileIframe").attr("src",ctx + '/socialorg/downFiles?id='+fileId);
