...
function loadData(){
	var options = {
			url : ctx+'/cover/coverPartyShowlFile',
...
function save(){
	$.ajax({
		url:ctx + '/cover/partyconstructioncancel',
		type:'post',
...
function uploadFile(modelTbId){
	posterProcess1({
		scriptData : {modelTbId :modelTbId ,model:"3",type:'2',action:'1'},
		script : 'fileUploads',
		fileExt:'*.*',
...
function downRow(fileId){
	$("#hiddeFileIframe").attr("src",ctx + '/cover/downFiles?id='+fileId);
