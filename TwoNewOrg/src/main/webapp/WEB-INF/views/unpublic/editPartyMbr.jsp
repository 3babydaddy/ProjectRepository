...
<head>
	<title></title>
	<script type="text/javascript" src="<c:url value='/resources/js/unpublic/editPartyMbr.js?version=${jsversion}'/>"></script>
</head>
<body >
	<form id="editForm">
					...
					<tr>
						...
						<td style="text-align: left;">
							<select id="partymbrInUnpublicIs" class="form-control" name="partymbrInUnpublicIs">
								...
								<option value="1" <c:if test="${main.partymbrInUnpublicIs == 1 }"> selected="selected"</c:if>>是</option>
								<option value="0" <c:if test="${main.partymbrInUnpublicIs == 0 }"> selected="selected"</c:if>>否</option>
							</select>
						</td>
					</tr>
				</table>
	   ...
	   <div class="panel-body" align="center">
	    	<label class="checkbox-inline">
			  <input type="checkbox" name="partymbrMiddleManagerIs" value="1" <c:if test="${main.partymbrMiddleManagerIs == 1 }"> checked="checked"</c:if>> 中层管理人员
			</label>
	    	<label class="checkbox-inline">
			  <input type="checkbox" name="partymbrOnMiddletechIs" value="1" <c:if test="${main.partymbrOnMiddletechIs == 1 }"> checked="checked"</c:if>> 中高级专业技术人员
			</label>
	    	<label class="checkbox-inline">
			  <input type="checkbox" name="partymbrFrontlineIs" value="1" <c:if test="${main.partymbrFrontlineIs == 1 }"> checked="checked"</c:if>> 生产经营一线职工
			</label>
	    	<label class="checkbox-inline">
			  <input type="checkbox" name="partymbrInVillageIs" value="1" <c:if test="${main.partymbrInVillageIs == 1 }"> checked="checked"</c:if>> 农村党员
			</label>
	   </div>
	   </div>
	   ...
	   <input type="hidden" name="unpublicOrgInfoId" id="unpublicOrgInfoId" value="${orgId }"/>
